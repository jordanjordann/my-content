...
import { useAnalysisQuery } from "@/lib/api/analyses";
import { AnalysisDetailHeader } from "../header/AnalysisDetailHeader";
import { AnalysisScorecard } from "./AnalysisScorecard";
import { AnalysisPerItem } from "./AnalysisPerItem";
import { AnalysisPatterns } from "./AnalysisPatterns";
import { AnalysisSuggestions } from "./AnalysisSuggestions";

/** Analysis detail page content: header, scorecard, per-item breakdown, patterns, and suggestions. */
export function DetailContent() {
  const params = useParams();
  const router = useRouter();
  const id = params.id as string;

  const { data, isPending, error } = useAnalysisQuery(id);
