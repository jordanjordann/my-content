...
| **A10** | 2026-08-20 | **§6.1 Sorting is WITHDRAWN in full by owner ruling. This is the first amendment that overturns an approved behaviour rather than correcting or completing one, and the owner approved the override knowing that.** **The ruling, verbatim in substance: *"Drop the sorting entirely. The default and only order is `updated_at DESC` — most recently analysed first."*** There is no sortable column, no header control, no sort arrow, no `aria-sort`, no default sort to choose and no `sortBy` / `sortDir` query parameter. The single fixed order is **`updated_at DESC` and nothing else — the owner considered a secondary sort key and DECLINED it** (see the rationale column). **R-S1** (absent values sink), **R-S2** (the sink group is visible, labelled and counted) and **R-S3** (no sort mixes denominators) are withdrawn with it, as are §6.1's sink labels — all three are rules *about sortable columns*, and with no sortable column they have nothing to bind. **R-D19 (§9.2.1) is moot**: the sort arrow it made the sole carrier of sorted state no longer exists. **Explicitly NOT withdrawn, and each verified in code at `9c91a1d` as independent of the sort:** **grouping** (the scored / scoreless / non-completed split and its labelled divider — `groupAnalysisRows` takes only rows and never reads a sort key), §6.2 filtering, §6.3 column visibility, §4.2/§4.3's header tooltip triggers and their **R-D6** sibling / **R-D12** count guarantees, and **R-D18**'s unconditional engagement header colour in its remaining idle / hover / focus-visible / sticky-scrolled states. **This amendment overrides §4.4 of [`DESIGN-3C-vs-their-usual-state-routing.md`](./DESIGN-3C-vs-their-usual-state-routing.md)**, which declined to change sort semantics, and **OR-8**, whose default key was `post_date` — the new key is `updated_at`, a different column producing a visibly different order. The withdrawn specification is preserved in place, struck and marked, so it is not rebuilt or re-raised as a finding. **No column, width, order, density, colour value, filter behaviour or user-facing string is added, removed or altered by this amendment** beyond the removal of the sort affordance itself. | **Owner ruling, 2026-08-20, on issue [#266](https://github.com/jordanjordann/my-content/issues/266); the owner chose none of the five options that issue laid out and removed the feature instead.** **His stated reasoning: the shipped sort *lies*.** It claims to order by *vs their usual* but orders by the **stored** multiplier. [PR #263](https://github.com/jordanjordann/my-content/pull/263) made the *displayed* multiplier **live-derived** when the stored column is `NULL`, which broke the pre-#263 equivalence "`perf_multiplier IS NULL` ≡ no multiplier displayed" that the sort key depended on **by construction**. The 2026-08-20 production census: **5 of the 7 rows that display a number have none stored**, so the majority of measured rows sort as absent while showing a real figure. The live value is **not computable inside the list query** — it needs `baseline.ts`'s per-pool median with self-exclusion, the 72h maturity floor and `metricFor()`, all of which run in app code *after* the page is selected — so ordering by it is in irreducible tension with `LIMIT`/`OFFSET`. **Removing the sort replaces a wrong answer with no answer, which is honest.** The owner explicitly **accepted the trade-off** that the two rows whose ordering is currently correct (`3b495116`, `7b6948fe`) also lose it. **The order key was CHANGED from `created_at` to `updated_at`, 2026-08-20, after the first revision of this amendment.** Reason: **re-analyze is an `UPDATE`, not an `INSERT`** (`lib/server/analysis/pipeline/index.ts:80-89`) and it sets `updated_at` but never `created_at`, so a re-analysed row would keep its original `created_at` and would **not** surface even though an analysis had just run for it. `updated_at` is also **already indexed** — `CREATE INDEX idx_analyses_updated_at ON analyses(updated_at DESC)`, `migrations/013_reach_unavailable_reason.sql:181`, verified present at `9c91a1d` — whereas `created_at` has none. **Every writer of `analyses.updated_at` is inside the analysis pipeline** — `pipeline/index.ts:85, 290, 362, 402`, all `WHERE id = ?`; the other `updated_at` writes in the codebase (`lib/server/fingerprint/repository.ts:142,200,319`, `lib/server/profiles/repository.ts:87`) target `profile_style_fingerprints` and `profiles`, not `analyses`. The one accepted consequence: **a row climbs the list as its own pipeline progresses**. **The secondary sort key was PROPOSED and DECLINED, 2026-08-20.** An earlier draft of this amendment and of #266 required `, a.id ASC`. **The owner considered it and declined it: the clause is exactly `ORDER BY a.updated_at DESC`.** `analyses.id` is an application-generated **UUIDv4** (`randomUUID()`, `pipeline/index.ts:1,68`, `TEXT PRIMARY KEY`) carrying **zero** ordering information — by rowid the ids run `b04d7d23…, dea20a90…, 46477354…, 66143a31…, ac3b449e…` — so `id` could only ever be an arbitrary-but-stable tiebreak. **Tie likelihood RE-DERIVED for `updated_at`, not carried over from `created_at`:** the four stage writes all hit the **same** row (`WHERE id = ?`) so they overwrite rather than multiply, and a tie needs two **different** rows whose *last* write lands in the same second; the batch path is **strictly sequential** (`app/api/analyze/route.ts:53-64`), which production confirms directly — row N's `updated_at` equals row N+1's `created_at` to the second (`16:12:23`, `16:13:18`, `16:14:08`) — with per-row runtime **43–118 s** and a smallest consecutive `updated_at` gap of **50 s**; production holds **12 rows, 12 distinct `updated_at`, zero `NULL`**. **Ties are therefore no more likely under `updated_at` than under `created_at`**, so the evidence the tiebreak was declined on still holds. **The residual risk the owner accepted, stated honestly:** `updated_at` is `TEXT NOT NULL DEFAULT (datetime('now'))` (`migrations/013_reach_unavailable_reason.sql:49`) — **one-second granularity, no `UNIQUE` constraint** — so two rows finishing in the same second (two tabs, a retry, or a future job queue) can collide, and under `LIMIT`/`OFFSET` pagination a non-deterministic tiebreak can make a row **duplicate across pages or vanish entirely**; **tied-row order is explicitly undefined and accepted**. No path can set `updated_at` to `NULL`, so SQLite's "NULLs last under `DESC`" does not arise. He was told this plainly and **accepted it as low-risk at current scale** (single user, 12 rows). The **3A job queue** on the roadmap would raise the likelihood; revisit then. Adding a `UNIQUE` constraint or another index is **not** the answer — that is a migration, and migrations are prohibited. **#266 must carry a comment at the `ORDER BY` site recording that the tiebreak was deliberately declined on 2026-08-20, so a future reader does not "fix" it without asking.** *(Owner ruling, 2026-08-20 session; issue #266; PR #263.)* |
...
### 6.1 Sorting — **WITHDRAWN, 2026-08-20, by owner ruling (amendment A10)**

> **Owner ruling, 2026-08-20:** *Drop the sorting entirely. The default and only order is
> `updated_at DESC` — most recently analysed first.*
>
> **There is no sorting in this table.** No column is sortable, no column header is a control, there
> is no sort arrow, no `aria-sort`, no default sort to choose and no `sortBy` / `sortDir` query
> parameter. The single, fixed, non-configurable order is exactly:
>
> ```sql
> ORDER BY a.updated_at DESC
> ```
>
> — most recently analysed first, and **nothing else**. **No secondary sort key.** No `IS NULL`
> leading key (`updated_at` is `NOT NULL`). No caller-controlled string anywhere near the clause.
>
> **Why `updated_at` and not `created_at` — owner change, 2026-08-20.** The earlier revision of this
> amendment specified `created_at DESC`. **Re-analyze is an `UPDATE`, not an `INSERT`**
> (`lib/server/analysis/pipeline/index.ts:80-89`) and it sets `updated_at` but never `created_at`, so
> a re-analysed row would keep its original `created_at` and would **not** move to the top even
> though an analysis had just run for it. The owner wants re-analysed rows to surface, and changed
> the key. `updated_at` is additionally **already indexed** —
> `CREATE INDEX idx_analyses_updated_at ON analyses(updated_at DESC)`
> (`migrations/013_reach_unavailable_reason.sql:181`, verified present at `9c91a1d`) — whereas
> `created_at` has **no** index, so this is also the cheaper read.
>
> **Every writer of `analyses.updated_at`, enumerated at `9c91a1d`.** All four live in
> `lib/server/analysis/pipeline/index.ts` — `:85` (re-analyze reset), `:290` (metadata/perf write),
> `:362` (`status = 'completed'`), `:402` (`status = 'failed'`) — and every one is scoped
> `WHERE id = ?`. **No non-analysis path writes it:** the other `updated_at = datetime('now')` writes
> in the codebase (`lib/server/fingerprint/repository.ts:142,200,319`,
> `lib/server/profiles/repository.ts:87`) target `profile_style_fingerprints` and `profiles`, **not**
> `analyses`, so a fingerprint recompute or a profile refresh does **not** reshuffle this table. The
> consequence the owner accepted is only this: **a row climbs the list as its own pipeline
> progresses** (pending → completed), rather than sitting still after insert as it did under
> `created_at`.
>
> **The secondary sort key was PROPOSED and DECLINED by the owner on 2026-08-20.** An earlier draft
> required `, a.id ASC`. He considered it and declined it. `analyses.id` is an application-generated
> **UUIDv4** (`randomUUID()`, `pipeline/index.ts:1,68`, `TEXT PRIMARY KEY`) carrying **zero**
> insertion-order information — by rowid the ids run `b04d7d23…, dea20a90…, 46477354…, 66143a31…,
> ac3b449e…` — so `id` could only ever be an arbitrary-but-stable tiebreak, never "last analysed
> first".
>
> **Tie likelihood, RE-DERIVED for `updated_at` — the `created_at` argument is not reused.** The
> original evidence (43–107 s gaps between consecutive **`INSERT`s**) was about `created_at` and does
> not transfer by assumption. Re-derived against Turso `lasa` on 2026-08-20 at `9c91a1d`:
>
> - **Multiple stage writes within one run cannot create a cross-row tie.** All four writes are
>   `WHERE id = ?` on the **same** row, so they *overwrite* that row's `updated_at`; they do not
>   multiply rows. A tie needs two **different** rows whose *last* write lands in the same second.
> - **The batch path is strictly sequential** (`app/api/analyze/route.ts:53-64` awaits the whole
>   `runAnalysis` pipeline per URL), so completions are spaced by the per-row runtime, not clustered.
>   Production confirms it directly: row N's `updated_at` equals row N+1's `created_at` **to the
>   second** (`16:12:23`, `16:13:18`, `16:14:08`) — the next row is inserted the instant the previous
>   one finishes. Per-row runtime measured **43–118 s**; the smallest gap between consecutive
>   `updated_at` values is **50 s**, the same order as under `created_at`.
> - **Production today: 12 rows, 12 distinct `updated_at`, 12 distinct `created_at`, zero `NULL`.**
>
> **Ties are therefore no more likely under `updated_at` than under `created_at` for the current
> workload**, and the evidence the owner declined the tiebreak on still holds.
>
> **Accepted risk, stated once and not to be re-argued.** `updated_at` is
> `TEXT NOT NULL DEFAULT (datetime('now'))` (`migrations/013_reach_unavailable_reason.sql:49`):
> **one-second granularity and no `UNIQUE` constraint.** It is `NOT NULL` and no code path sets it
> `NULL` — all four writers assign `datetime('now')` — so the SQLite "NULLs last under `DESC`"
> question does not arise. Two rows finishing in the same second — two tabs, a retry, or a future job
> queue — can still collide, and under `LIMIT`/`OFFSET` pagination a non-deterministic tiebreak can
> make a row **duplicate across pages or vanish entirely**. **Tied-row order is explicitly undefined
> and accepted.** The owner was told this plainly and **accepted it as low-risk at current scale**
> (single user, 12 rows). The **3A job queue** on the roadmap would raise the likelihood; revisit
> then. **Do not** add a `UNIQUE` constraint or another index — that is a migration, and migrations
> are prohibited. The implementation must carry a comment at the `ORDER BY` site recording that the
> tiebreak was deliberately declined, so a future reader does not "fix" it without asking.
>
> **Reasoning, as stated by the owner:** the shipped sort *lies*. It claims to order by multiplier but
> orders by the **stored** multiplier, and since #263 made the displayed multiplier live-derived, the
> 2026-08-20 production census shows the majority of the rows that display a number have none stored.
> Removing the sort replaces a wrong answer with **no answer**, which is honest. The owner explicitly
> accepted the trade-off that the two currently-correct rows also lose their correct ordering. He also
> explicitly chose full removal over the alternative of keeping a single date-column sort toggle.
>
> **This overrides §4.4 of [`DESIGN-3C-vs-their-usual-state-routing.md`](./DESIGN-3C-vs-their-usual-state-routing.md)**,
> which declined to change sort semantics, and **supersedes OR-8**, whose default key was `post_date`.
> **OR-8 is superseded, not merely overridden**: the new key is `updated_at` — the last-analysed
> timestamp, a *different column* from the creator's publish date, deliberately so, producing a
> visibly different order (`dea20a90`: `post_date` 2024-12-26 vs `updated_at` 2026-08-17). Do not
> silently keep `post_date`. Ticketed as [#266](https://github.com/jordanjordann/my-content/issues/266).
>
> **What is NOT withdrawn:** **grouping**. The scored / scoreless / non-completed split and its
> labelled divider are a separate axis and are untouched — the grouping code never reads a sort key.
> §6.2 filtering, §6.3 column visibility, §9.2.1's **R-D18** engagement header colour (in its
> remaining idle / hover / focus-visible / sticky states), and §4.2's engagement tooltip triggers all
> stand. **R-D19 is moot**: the sort arrow it made the sole carrier of sorted state no longer exists.
>
> Everything below is **preserved as the withdrawn specification** so that no part of it is rebuilt or
> re-raised as a finding. **Nothing below may be built.**

**~~WITHDRAWN.~~** Sortable: **Creator, Posted, Counts (by reach), Content score, Performance score, vs their usual, Eng./reach, Eng./followers.** Default sort: **Posted, descending.**
