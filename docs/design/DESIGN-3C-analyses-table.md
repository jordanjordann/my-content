...
> `created_at DESC` — newest analysis first.*
>
...
> ```
>
> — newest analysis run first, and **nothing else**. **No secondary sort key.** No `IS NULL` leading
> key (`created_at` is `NOT NULL`). No caller-controlled string anywhere near the clause.
>
> **The secondary sort key was PROPOSED and DECLINED by the owner on 2026-08-20.** An earlier draft
> required `, a.id ASC`. He considered it and declined it. The evidence he ruled on: the batch
> analyse path is **strictly sequential** (`app/api/analyze/route.ts:53-64` awaits the whole
> `runAnalysis` pipeline per URL; the row is `INSERT`ed at the top of each run,
> `lib/server/analysis/pipeline/index.ts:90-97`), so consecutive rows in one batch land **43–107
> seconds** apart; **no insert path ever passes `created_at`**, so it always takes the per-statement
> DB default `datetime('now')` and the "one `now()` reused across a batch" failure mode does not
> exist; production held **12 rows with 12 distinct `created_at`**; and `analyses.id` is an
> application-generated **UUIDv4** (`randomUUID()`) carrying **zero** insertion-order information, so
> it could only ever be an arbitrary-but-stable tiebreak, never "last inserted first".
>
> **Accepted risk, stated once and not to be re-argued.** `created_at` is
> `TEXT NOT NULL DEFAULT (datetime('now'))`: **one-second granularity, no `UNIQUE` constraint, no
> index.** Two *concurrent* inserts — two tabs, a retry, or a future job queue — can collide in the
> same second, and under `LIMIT`/`OFFSET` pagination a non-deterministic tiebreak can make a row
> **duplicate across pages or vanish entirely**. The owner was told this plainly and **accepted it as
> low-risk at current scale** (single user, 12 rows). The **3A job queue** on the roadmap would raise
> the likelihood; revisit then. **Do not** add a `UNIQUE` constraint or an index on `created_at` —
> that is a migration, and migrations are prohibited. The implementation must carry a comment at the
> `ORDER BY` site recording that the tiebreak was deliberately declined, so a future reader does not
...
> **OR-8 is superseded, not merely overridden**: the new key is `created_at` — the analysis-run date,
> a *different column* from the creator's publish date, deliberately so, producing a visibly different
> order (`dea20a90`: `post_date` 2024-12-26 vs `created_at` 2026-08-17). Do not silently keep
> `post_date`. Ticketed as [#266](https://github.com/jordanjordann/my-content/issues/266).
>
