...
> parameter. The single, fixed, non-configurable order is **`created_at DESC, id ASC`** — newest
> analysis run first, with `id` as a deterministic tiebreak because `created_at` is
> `TEXT NOT NULL DEFAULT (datetime('now'))`: **one-second granularity, no `UNIQUE` constraint**, so
> ties are possible and an unstable tiebreak would duplicate or drop rows across paginated pages.
>
...
> 2026-08-20 production census shows **5 of the 7 rows that display a number have none stored**.
> Removing the sort replaces a wrong answer with **no answer**, which is honest. The owner explicitly
> accepted the trade-off that the two currently-correct rows also lose their correct ordering.
>
> **This overrides §4.4 of [`DESIGN-3C-vs-their-usual-state-routing.md`](./DESIGN-3C-vs-their-usual-state-routing.md)**,
> which declined to change sort semantics, and **OR-8**, whose default key was `post_date`. The new
> key is `created_at` — a different column, and a visibly different order.
