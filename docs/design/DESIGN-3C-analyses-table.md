...
### 9.2.1 The two engagement column headers — the colour, and every state it has to survive *(R-D18 / R-D19, RULED 2026-08-14)*
...
- **§9.5 is unchanged and still holds.** Greyscale loses nothing: §4's distinguisher 3 already rules the colour a **redundant** channel, and after R-D19 both of the header's interaction affordances are shape, not hue.

#### R-D19 — the sorted state is carried by the arrow alone; hover gets a non-colour affordance
...
- **The `focus-visible` ring is unchanged on all headers**, and remains the keyboard affordance. R-D19 adds nothing to the keyboard path and removes nothing from it.
- **The two engagement headers also carry the §4.2 tooltip trigger (R-D5…R-D11).** That trigger is a **separate sibling `<button>`** with its own accessible name and its own focus ring; R-D18 and R-D19 bind the **sort control only** and change nothing about it. A hover over the sort label must not open the tooltip, and a hover over the tooltip trigger must not underline the sort label.
