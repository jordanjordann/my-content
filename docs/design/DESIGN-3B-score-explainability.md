...
| 3 | **Cause not determinable** — no usable inputs *and* the hidden-counts flag is absent from the payload | **`CAUSE_NOT_DETERMINABLE`** — exists and is live; see the note under this table | **`No performance data published`** | `Instagram published no view, like or comment data for this post. We can't tell whether the creator turned the counts off or whether Instagram simply didn't return them — so we're not going to guess.` |
