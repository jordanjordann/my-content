...
| **B11** | 2026-08-18 | **Record-keeping and mockup-fidelity pass. No approved copy string is written, edited or withdrawn, and no design decision is taken.** (a) **Issue [#209](https://github.com/jordanjordann/my-content/issues/209)'s footer question is already answered by B6 and is recorded as closed, not re-opened.** The ticket's premise — *"the footer is unconditional today"* — was true when it was filed and is **stale**: §4.5.1 specified F1/F2 on 2026-08-13 and PR [#232](https://github.com/jordanjordann/my-content/pull/232) shipped it (`scoreExplainFooter(date, coldStartBucketNoun)`, commit `c552db8`). F1 and F2 are **ratified as shipped** and are not redesigned. (b) **§4.5.1 gains one binding rule: there is no third footer string and no approved abbreviation of F1.** (c) **Both mockups are corrected to the F1/F2 spec — four footers in total.** In `3c-analyses-table-mockup.html`, **three of the four popover footers were the unapproved short form** `Measured {date} and frozen.`: the **cold-start** row's (which is the one row where **F2** is mandatory, and which additionally named the bucket **`video carousels`**, a noun outside the settled set that contradicted its own row's cell), the follower-denominated row's, and the provisional/`Early` row's. The cold-start one now renders **F2** with the cell's own noun `carousels`; the other two render **F1** in full; the fourth was already F1 and is only annotated. In `3b-score-explainability-mockup.html`, the follower-denominated panel carried the same short form and is restored to **F1**, and the file gains a side-by-side **F1/F2 panel** so the sign-off surface actually shows the variant this ticket is about. (d) **OR-25 is recorded as settled in §4.5.1** — *the prose guard fails loudly with no repair retry*, **CONFIRMED** — and the `MEASURED`-baseline question it gated is marked **unblocked but not commissioned**, needing its own ticket. | (a) A ticket that describes shipped work as outstanding gets actioned twice, and the second pass would have re-written approved, live copy for no reason. (b)+(c) The two mockups carried the short form `Measured {date} and frozen.` four times between them — it drops **`and don't update`**, which is the entire load-bearing clause (§4.5: *"the single sentence that prevents the most likely wrong client statement"*), and a mockup is what a developer copies from. The `video carousels` noun is the **R-13.4.1 / OR-9** set (`reels` / `carousels` / `Shorts` / `videos` / `posts`) with an invented member, and B6's own binding rule says a footer noun disagreeing with its cell's noun *"is a bug"* — here the **popover** disagreed with the cell in its own row. Restoring approved copy into a mockup is a fidelity act, not a copy decision. (d) OR-25 sat `CONFIRMED` in `TDD` §0.6 from 2026-08-06, was **re-opened as a live pain point** on 2026-08-13 (`HANDOFF-2026-08-13.md` §7, *"Raised, not decided"*), and is now re-confirmed unchanged — so the design-side consequence is written down where the deferral is written down. **`TDD` §14.8a and `HANDOFF-2026-08-13.md` §7 still carry the stale *"under consideration"* framing and are flagged for the tech lead, not edited here** (the `DESIGN-3C` A5 precedent: a design document does not edit the `TDD`). **No stored value, no enum, no colour value, no component, no column, no width, no density, no sort, filter or interaction changes, and no new user-facing string is introduced.** *(Issue #209; B6; PR #232.)* |
...
- **There is no third footer string, and no approved abbreviation of F1.** *(Added by amendment B11.)* `Measured 12 Jul 2026 and frozen.` is **not** F1 in short form — it drops *"and don't update"*, which is the clause that does the whole of the work, and it is the sentence §4.5 calls *"the single sentence that prevents the most likely wrong client statement."* Both mockups carried that shorthand and are corrected under B11. Two variants exist; the count is closed.
...
- **Deferred, deliberately, and not designed here:** whether to surface that a better baseline now exists for a `MEASURED` row (#209's second half). It waits on OR-25 and nothing in this sub-section anticipates it.
  ...
  > **OR-25 HAS LANDED — 2026-08-18, amendment B11. The gate is lifted; the work is not commissioned.** **OR-25 is settled as `CONFIRMED`: the prose guard fails loudly with NO repair retry.** A violation burns a billed Gemini call and fails the analysis, and the owner has ruled that cost understood and accepted — a loud failure beats a quietly unqualified percentage in a client deck. The ruling is recorded in `TDD` §0.6 (OR-25) and `TDD` §13 (E7); it was **re-opened as a live pain point on 2026-08-13** (`docs/HANDOFF-2026-08-13.md` §7 item 1, *"Raised, not decided"*) and has now been **re-confirmed unchanged**. It is settled fact and is not reopened here.
  >
  > **What that means for the bullet above.** The `MEASURED`-baseline question was deferred *behind* OR-25 because a "recompute this" affordance would need a second billed call, and whether a failed call may be retried decides whether such an affordance can be offered honestly. With no retry, **a second call that fails leaves the row with no recourse** — so any future surface here must be a *statement of fact* about the baseline, never a button that can silently strand the user. **The gate is lifted. The design is still not commissioned and is still not designed here** — the owner has not asked for it. It needs its own ticket before any of it is drawn.
  >
  > **Two stale conditionals this supersedes, both outside this document and flagged rather than edited** (the `DESIGN-3C` A5 precedent: a design document does not edit the `TDD`): **`TDD` §14.8a** still reads *"OR-25's no-retry stance is under separate consideration"*, and **`docs/HANDOFF-2026-08-13.md` §7** still reads *"UNDER ACTIVE OWNER CONSIDERATION — not settled either way."* Both are false as of 2026-08-18. Flagged for the tech lead. A conditional that carries a stated default is exactly the shape that gets "resolved" in the wrong direction by the next reader.
