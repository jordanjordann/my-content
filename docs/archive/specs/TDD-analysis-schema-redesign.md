...
1. ~~**`@google/generative-ai@^0.24.1` is the deprecated legacy SDK.** …the implementer must confirm the contract is expressible, and escalate to a `@google/genai` migration if not.~~ **[RESOLVED 2026-07-22 — this item is closed; do not re-investigate.]** The escalation happened and landed: #75 / PR #80 migrated the repo to **`@google/genai@^2.13.0`** and **removed `@google/generative-ai` entirely**. Build against `@google/genai` only. Expressibility is **settled** — native enums, arrays of enums, nested objects with `required`, and `nullable` all work, and the body comes back directly `JSON.parse`-able (evidence in #66 and `.claude/context/verified-facts.md`). The one real limitation stands: the schema's integer type has **no min/max**, so 1–5 range enforcement lives in the validation layer (#68). Two SDK-shape traps carry forward: **`response.text` is a property, not a method**, and the file-upload path is `@google/genai`'s own — `GoogleAIFileManager` and the `@google/generative-ai/server` entry point no longer exist.
...
1. ~~`responseSchema` support and expressiveness on `@google/generative-ai@0.24.1`~~ **[CLOSED 2026-07-22.]** The repo is on **`@google/genai@^2.13.0`** (#75 / PR #80); the legacy SDK is removed. Expressiveness is settled — see §4.2 item 1. Nothing to verify here beyond a mechanical re-confirm against the new SDK's typings inside #66.
2. Real `maxOutputTokens` headroom for the full contract on a long carousel, with thinking tokens counted (§4.2). **Still open — no live Gemini call has been made since the SDK migration.**
3. ~~The ScrapeCreators carousel **video**-child payload shape — still uncaptured~~ **[CLOSED 2026-07-22 by PR #84.]** Captured: `.claude/context/fixtures/scrapecreators-instagram/ig_carousel_mixed_video_and_image_10_slides.json` (7 video + 3 image children). It **falsified** the modelled shape — no `video_duration`, no `clips_music_attribution_info`, no `thumbnail_src` on video children, plus an undocumented `dash_info`. #71 owns applying the fix; validate against the fixture, do not re-derive.
