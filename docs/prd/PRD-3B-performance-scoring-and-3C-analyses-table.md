...
**OR-26** — from **two origins, not one**: six of the seven values are resolved in `judgement.ts`, while
`REACH_NOT_ON_FIRST_SLIDE` is decided upstream in `reach.ts` and only *mapped* onto the enum in
`judgement.ts`. **TDD §4, *"`unavailableReason` has TWO origins, not one"*, is the authority on which value
sits on which path; it is deliberately not restated here.**
